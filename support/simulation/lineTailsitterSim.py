...
if __name__=="__main__":
    ...
    tail.setWind([0., 0., 0.,])
    mass = 0.564
    tail.setInertia(m=mass, I=1*np.diag([5.73e-3, 0.625*1.35e-3, 0.5*5.43e-3]))
    ...
    for i in tqdm(range(int(T / dt)), target_looptime=dt_rt):
        if args.learn and sim.t > 3.0:
            ...
            sil.mockup.sendPositionSetpoint( [0., 0., -3.], 0. )

        if not args.throw and sim.t > 2.5 and not armed and sil is not None:
            sil.mockup.arm() if sil else None
            sil.mockup.sendPositionSetpoint( [0., 0., -3.], 0. )
            armed = True

        # if not start_trajectory and sim.t > 8. and sil is not None:
        ...
        if sim.t > 7. and sil is not None:
            # sil.mockup.sendPositionSetpoint( [20., 0., -1.5], 0. )
            sil.mockup.sendPositionSetpoint( [50., 0., -3], 0. )
        if sim.t > 13. and sil is not None:
            # sil.mockup.sendPositionSetpoint( [20., 0., -1.5], 0. )
            sil.mockup.sendPositionSetpoint( [50., 3., -3], 0. )
