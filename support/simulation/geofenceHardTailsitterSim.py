...
from PyNDIflight.crafts import Tailsitter, TailsitterPhi, TailsitterPhiFF, IMU
...
if __name__=="__main__":
    ...
    parser.add_argument("--time", required=False, type=float, default=1000., help="Total simulation time in seconds")
    ...
    # freestream elevon effectiveness calculations
    F_hover = mass*9.81 / 2
    # N = kg m / s^2 = kg / m3 * m3 / s * m / s
    # N = kg m / s^2 = RHO * Vdot * Delta V 
    # F = RHO * A * Vdisk * Delta V
    # random actuator disk model   V = 2 Delta V
    # F = RHO * A * Vdisk**2/2
    # Vdisk = sqrt( 2F / (RHO * A) )
    Vdisk2_hover = 2*F_hover / (1.225 * (5*25.4e-3/2)**2 * np.pi)

    # F_hover is also = k*w_hover**2
    w2_hover = F_hover / k

    # make a random assumption that the freestream affects as much of the elevon
    # surface area as the prop downstream. I may be more, but not the whole area,
    # since some is always shadowed by the propeller
    # this leads to the assumptions that at cd*w**2 at hover should be as large
    # as cd_qinf*qinf at qinf = 0.5 * RHO * Vdisk_hover**2 
    cd_qinf = cd * w2_hover / (0.5 * 1.225 * Vdisk2_hover)

    tail.setPhiModel(phi, Phi)
    tail.setElevonModel(cd, cdd, cddd, cd_qinf, d0)
    ...
    #%% initial conditions
    tail.setPose(x=[0, 0, -0.1], q=[0.707, 0., 0., 0.707])
    ...
    takeoff = False
    traj_inited = False
    traj_activated = False
    traj_recovered = False
    back_to_center = False
    violate_again = False
    for i in tqdm(range(int(T / dt)), target_looptime=dt_rt):
        if sil is not None:
            if not args.throw and not armed and sim.t > 2.1:
                sil.mockup.arm()
                armed = True

            if not takeoff and sim.t > 3.0:
                sil.mockup.sendPositionSetpoint( [0., 0., -1.0], 0. )
                print("Takeoff!")
                takeoff = True

            if not traj_inited and sim.t > 6.0:
                # start trajectory tracking at 8*0.5 = 4m/s target speed
                sil.mockup.sendKeyboard('1')
                print("Init!")
                traj_inited = True

            if not traj_activated and sim.t > 10.0:
            ...
            # if traj_activated and not traj_recovered and not sil.mockup.lib.nn_is_active():
            #     print("Recovery initiated by INDIflight")
            #     sil.mockup.disableRcBox(boxId.BOXNNCTL)
            #     traj_recovered = True

            if not traj_recovered and sim.t > 15.0:
                sil.mockup.sendKeyboard('8')
                print("Recovery initiated by simulation command!")
                traj_recovered = True

            if not back_to_center and sim.t > 20.:
                sil.mockup.sendPositionSetpoint( [0., 0., -1.0], 0. )
                print("Back to center")
                back_to_center = True

            if not violate_again and sim.t > 25.:
                sil.mockup.sendPositionSetpoint( [0., 10., -1.0], 0. )
                print("Violate geofence again, then violate hardkill")
                violate_again = True

            if sim.t > 32.:
                # keep on sending
                sil.mockup.sendPositionSetpoint( [0., 2., -40.0], 0. )
