...
if __name__=="__main__":
    ...
    #%% Generate craft
    mc = MultiRotor(Nr=6)
    mc.setInertia(m=0.321, I=np.diag([0.8e-3, 0.83e-3, 1.1e-3]))
    ...
    for i in range(6):
        mc.setRotor(i, X=X[i], k=1.88e-7, cm=direc[i]*0.01, wmax=4900., tau=0.025, kESC=0.5, I=6.3e-7, ax=axes[i])
    ...
    for i in tqdm(range(int(T / dt)), target_looptime=dt_rt):
        if not args.throw and sim.t > 1.5:
            sil.mockup.arm() if sil else None

        if not start_trajectory and sim.t > 3. and sil is not None:
            sil.mockup.sendPositionSetpoint( [0., 0., -1.5], np.pi/2 )
            sil.mockup.enableFlightMode(flightModeFlags.ANGLE_MODE | flightModeFlags.POSITION_MODE)

        if sim.t > 6.:
            sil.mockup.sendPositionSetpoint( [1., 0., -1.5], np.pi/2 )
