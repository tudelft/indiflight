...
--deploy=<DEPLOY_PATH>          # upload the build-aarch64-linux-gnu directory to the pi using rsync after building. Requires project() to be set in CMakeLists.txt and it will be uploaded to <DEPLAY_PATH>/<CMAKE_PROJECT_NAME>/build-aarch64-linux-gnu. Also requires --env-file pointing to remote.env (with REMOTE_NAME or REMOTE_IP, REMOTE_USER, REMOTE_PASSWORD)
...
Pro-tip: add the following alias to your `~/.bashrc` so that you can omit the docker boilerplate (add `--env-file /path/to/remote.env` if you use `--deploy`):
```bash
