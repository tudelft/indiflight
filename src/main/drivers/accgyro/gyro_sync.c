...
uint16_t gyroSetSampleRate(gyroDev_t *gyro)
{
    ...
    switch (gyro->mpuDetectionResult.sensor) {
        ...
        case LSM6DSO_SPI:
            ...
            accSampleRateHz = 6664;
            break;
#endif
        default:
            gyro->gyroRateKHz = GYRO_RATE_8_kHz;
            gyroSampleRateHz = 8000;
            accSampleRateHz = 8000; // MPU6000 IMU is actually 1000Hz, be we have no way of knowing which sample is which, so we run at 8000. meh
            break;
    }
