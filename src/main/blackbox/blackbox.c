...
static const blackboxDeltaFieldDefinition_t blackboxMainFields[] = {
    ...
    {"extQuat",      0, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(SIGNED_VB), CONDITION(POS)},
...
static void loadMainState(timeUs_t currentTimeUs)
{
    ...
    blackboxCurrent->extQuat[0] = lrintf(extPosNed.quat.w * UNIT_FLOAT_TO_SIGNED16VB);
